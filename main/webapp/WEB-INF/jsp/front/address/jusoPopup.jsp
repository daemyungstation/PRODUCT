...
<script language="javascript">
   ...
   //document.domain = "dev-product.daemyungimready.com";
    window.onmessage = function(e){
        console.log(e);
    }
    function init(){
        var url = location.href;
        var confmKey = "U01TX0FVVEgyMDIyMTEwMzE0MjMzMjExMzE4Mjc=";//승인키
        ...
        if(inputYn != "Y"){
            ...
            /** API 서비스 제공항목 확대 (2017.02) **/
            opener.postMessage( { "messageType": "address", "data": ["<%=roadFullAddr%>","<%=roadAddrPart1%>","<%=addrDetail%>", "<%=roadAddrPart2%>","<%=engAddr%>"
                , "<%=jibunAddr%>","<%=zipNo%>", "<%=admCd%>", "<%=rnMgtSn%>", "<%=bdMgtSn%>", "<%=detBdNmList%>"
                , "<%=bdNm%>", "<%=bdKdcd%>", "<%=siNm%>", "<%=sggNm%>", "<%=emdNm%>", "<%=liNm%>", "<%=rn%>", "<%=udrtYn%>"
                , "<%=buldMnnm%>", "<%=buldSlno%>", "<%=mtYn%>", "<%=lnbrMnnm%>", "<%=lnbrSlno%>", "<%=emdNo%>" ] } , "*"
            );
            <%--opener.CommonCtrl.JusoCallBack("<%=roadFullAddr%>","<%=roadAddrPart1%>","<%=addrDetail%>", "<%=roadAddrPart2%>","<%=engAddr%>"--%>
            <%--    , "<%=jibunAddr%>","<%=zipNo%>", "<%=admCd%>", "<%=rnMgtSn%>", "<%=bdMgtSn%>", "<%=detBdNmList%>"--%>
            <%--    , "<%=bdNm%>", "<%=bdKdcd%>", "<%=siNm%>", "<%=sggNm%>", "<%=emdNm%>", "<%=liNm%>", "<%=rn%>", "<%=udrtYn%>"--%>
            <%--    , "<%=buldMnnm%>", "<%=buldSlno%>", "<%=mtYn%>", "<%=lnbrMnnm%>", "<%=lnbrSlno%>", "<%=emdNo%>");--%>
            window.close();
