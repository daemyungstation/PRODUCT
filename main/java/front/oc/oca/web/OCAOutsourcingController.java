...
public class OCAOutsourcingController extends EmfController {
    ...
    private BankingService doznBankingService;


//	@Resource(name = "inicisBankingService")
//	private BankingService inicisBankingService;
	...
	public String getAgreeView(HttpServletResponse response, HttpServletRequest request, EmfMap emfMap, ModelMap modelMap, @PathVariable String agentnm) throws Exception
	{
		...
		if( "Cuckoo-home".equalsIgnoreCase(agentnm)) {
			agentnm = "Cuckoo";
		}
...
*/				
	...
	public String getOutsourcingLGB2BWrite(HttpServletRequest request, EmfMap emfMap, ModelMap modelMap) throws Exception
	{
		...
		*/
		RequestContextHolder.getRequestAttributes().setAttribute("productHomePath", "/"+ agentnm +"/write.do", RequestAttributes.SCOPE_SESSION);
		// 동의/인증화면 사용여부
		String useOutsourcingAgree = EgovProperties.getProperty("Globals.useOutsourcingAgree");
	...
	{ 
		...
		else
		{
			if(agentnm.equals(tempAgentnm))
			{
				...
				if (agentnm.equals("LGU")){
					url = "front/oc/oca/OCAOutsourcingWriteLGU_new.front3";
				} else{
					url = "front/oc/oca/OCAOutsourcingWrite"+agentnm+".front2";	
				}
			}
	...
	public String insertOutsourcingInf(EmfMap emfMap, HttpServletRequest request, ModelMap modelMap) throws Exception
	{
		...
		{	
			...
			else
			{
				...
				if("SMART".equals(tempAgentnm) || "SMARTMOBILE".equals(tempAgentnm) || "SMARTHOMEPLUS".equals(tempAgentnm) || "CUCKOO".equals(tempAgentnm)) {
				...
				if ("0".equals(overrapMap.getString("cnt"))) {
					if ("Y".equals(useOutsourcingAgree)) {
						...
						if("WEDDING".equals(emfMap.get("b2bStts")) || "HANSSEM".equals(emfMap.get("b2bStts"))){
	...
	@RequestMapping(value={"/outsourcing/complete.do", "/EXPIRATION/complete.do"})
	...
	@RequestMapping(value="/LGU/complete.do")
	public String getLGUCompleteView(EmfMap emfMap, ModelMap modelMap) throws Exception
	{
		return "front/oc/oca/OCAOutsourcingLGUComplete.front4";
	}
...
*/
	...
	public ModelAndView agentAuthCheck(EmfMap paramMap, ModelMap modelMap, HttpServletRequest request) throws Exception {
		...
		try {
			log.debug("1 : " + paramMap.getString("sllrCtel"));
			log.debug("2 : " + paramMap.getString("birthDay"));
			log.debug("3 : " + paramMap.getString("agentEmpNm"));
			log.debug("4 : " + paramMap.getString("ci"));
	...
	public ModelAndView agentCdCheck(EmfMap paramMap, ModelMap modelMap, HttpServletRequest request) throws Exception {
		...
		try {
			...
			log.debug("agentCd : " + paramMap.getString("agentCd"));
...
//			EmfMap overrapMap = oCAOutsourcingService.getOverrapSid(emfMap);
	 ...
	 * @throws 비지니스 로직이나 DAO 처리 중 에러가 발생할 경우 Exception을 Throw 한다.
     */    
	@RequestMapping(value="/LGUElectronic/insert.do", method=RequestMethod.POST)
	public String insertOutsourcingInf_lgu_ele(EmfMap emfMap, HttpServletRequest request, ModelMap modelMap) throws Exception {
		String url = "error/blank.error";
		...
		try {
			...
			// 중복가입 검사
			String payMthd = request.getParameter("pay");
			String rdpMstBnkCd = null;
			String expireDt = null;
			if(payMthd != null) {
				String callTypeEle = request.getParameter("callTypeEle");
				String callDateEle = request.getParameter("callDateEle");
				String tel_no = request.getParameter("tel_no");
				tel_no = phone_format(tel_no);
				payMthd = payMthd.toString().toUpperCase();
				if(payMthd.equals("CARD")){
					String cardMon = request.getParameter("cardMon");
					String cardYear = request.getParameter("cardYear");
					expireDt = cardYear + cardMon;
				}
				if (payMthd.equals("CMS")) {
					rdpMstBnkCd = request.getParameter("rdpMstBnkCd");
				}
				emfMap.put("hp", tel_no);
				emfMap.put("callType", callTypeEle);
				emfMap.put("callDate", callDateEle);
			}
			EmfMap overrapMap = oCAOutsourcingService.getOverrapSidEle(emfMap);
			String cardNum = request.getParameter("cardNum");
			String accountNum = request.getParameter("accountNum");
			String sex = request.getParameter("sex");
			String agentEmpTel = request.getParameter("agentEmpTel");
			agentEmpTel = phone_format(agentEmpTel);
			String key = "daemyung_product_electronic_0820";
			if(sex!= null){
				sex = "000"+sex;
			}
			
			
			emfMap.put("payMthd", payMthd);
			emfMap.put("agentEmpTel", agentEmpTel);
			emfMap.put("accountNum", accountNum);
			emfMap.put("cardNum", cardNum);
			emfMap.put("expireDt", expireDt);
			emfMap.put("rdpMstBnkCd", rdpMstBnkCd);
			emfMap.put("sex", sex);
			
			if("0".equals(overrapMap.getString("cnt"))) {
				isOk = oCAOutsourcingService.insertOutsourcingInfEle(emfMap, request);
			} else {
				isOk = false;
			}
	...
	@RequestMapping(value =  "/outsourcing/phoneAuthLGU.do", method=RequestMethod.POST)
	public String getPhoneAuthLGU(EmfMap emfMap, ModelMap modelMap, HttpServletRequest request) throws Exception {
		return "front/okname/hs_cnfrm_popup6.pop";
	}

	@RequestMapping({ "/outsourcing/phoneAuthLGU2.do" })
	public String getPhoneAuthLGU3(EmfMap emfMap, ModelMap modelMap) throws Exception {
		return "front/okname/hs_cnfrm_popup7.pop";
	}

	@RequestMapping(value =  "/outsourcing/getPhoneAuthInfLGU.ajax", method=RequestMethod.POST)
	public ModelAndView getPhoneAuthInfLGU(EmfMap emfMap, ModelMap modelMap, HttpServletRequest request) throws Exception {
		String url = "error/blank.error";
		String ci = "";
		String di = "";
		String name = "";
	...
	// 빌키발급 예시
	//@RequestMapping(value =  "/outsourcing/cardAuthLGU.do", method=RequestMethod.POST)
	...
	public ModelAndView cardAuthLGU(EmfMap emfMap, HttpServletRequest request) throws Exception {
		...
		String birthday = (emfMap.getString("birthday")).substring(2);
		...
		inicisPay.setData("buyerName", emfMap.getString("buyerName"));
		inicisPay.setData("buyerEmail", "k");
		inicisPay.setData("buyerTel", emfMap.getString("hp"));
		inicisPay.setData("cardNumber", emfMap.getString("cardNum"));
		inicisPay.setData("cardExpire", cardExpire);
		inicisPay.setData("regNo", birthday);
...
//		json.addObject("tid", tid);
		json.addObject("billKey", billKey);
		
		return json;
	}
	
	@RequestMapping(value = "/outsourcing/bankAccount2021.ajax")
    public ModelAndView checkBankAccountAjax(EmfMap emfMap, HttpServletRequest request) throws Exception {
        ...
        String bankCode = "0" + emfMap.getString("bankCode");
        emfMap.put("accountNum", accountNum);
        emfMap.put("bankCode", bankCode);
        Map<String, Object> result = doznBankingService.inquireDepositor(emfMap);
        //Map<String, Object> result = doznBankingService.inicisDepositor(emfMap);
	...
	public ModelAndView checkBankAccountAjaxInicis(EmfMap emfMap) throws Exception {
		JSON json = new JSON();
		String accountNum = emfMap.getString("accountNum");
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/address/jusoPopup.do")
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/address/jusoTest.do")
