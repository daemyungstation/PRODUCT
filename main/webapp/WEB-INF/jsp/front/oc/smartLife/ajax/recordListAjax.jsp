        ...
        <c:when test="${not empty emfMap.record}">
            <c:forEach items="${emfMap.record}" var="record" varStatus="status">
                <tr>
                    ...
                    <%--<td>${record.authCd}</td>--%>
                    <%--<td>${record.authCd ? 'Y' : 'N'}</td>--%>
                    <%--<td>${record.authDt}</td>--%>
                    <td>
                        <c:if test="${record.accStat ne '해약'}">
                            <c:set var="sum" value="${sum + (record.sdpAlow eq null ? 0 : record.sdpAlow)}" />
                        </c:if>
                        <fmt:formatNumber value="${(record.sdpAlow eq null) ? 0 : record.sdpAlow}" type="currency" />
                    </td>
                    <td></td>
                    <td></td>
                </tr>
