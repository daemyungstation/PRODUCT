	...
	<div class="log-popup">
		<div class="log-popup-in">
			...
			<div class="log-popup-box">
				<p class="txt-info">
					<b><콜센터 임시 휴업 긴급 공지사항></b><br/><br/>
					안녕하십니까?<br/>
					‘코로나19’로 인한 콜센터 임시 휴업과 관련하여 긴급 공지 안내드립니다.<br/><br/>
					스마트라이프 가입센터 건물에 확진자 발생으로<br/>방역 작업을 위해 9월 19일(토)임시 휴업이 결정되었습니다.<br/><br/>
					상담원의 안전과 추가 확진자 발생을 막기 위한 조치이오니 양해 부탁드립니다.<br/><br/>
					9월 19일(토) 접수된 DB는 익일 20일(일)부터<br/>순차적으로 아웃콜 진행될 예정입니다.<br/><br/>
					감사합니다.
				</p>
			</div>
			<div class="center">
...
<%-- 						</c:if> --%>
						...
						<c:if test="${info.agentCdYn eq 'Y'}">
							<tr>
								...
								<td>
									...
									&nbsp;&nbsp;
									<span class="red" id="resultCheckAgent">※ 매장 코드 형식(삼성판매, AK : Z***, SDP : D***)</span>
				...
				<%-- 		<c:if test="${info.agentCdYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">매장 코드</span>
								</th>
								<td>
				...
				<table summary="">
					...
					<tbody>
						...
						<c:if test="${info.fusionPrdctYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">결합상품 선택</span>
								</th>
								<td>
						...
						<c:if test="${info.nameYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">고객명</span>
								</th>
								<td>
									<input type="text" name="name" title="고객명" class="inputType1" maxlength="20" onkeypress="javascript:trimCustNm(this) return this.value;" />
								</td>
							</tr>
						</c:if>
						...
						<c:if test="${info.storeNmYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">점포명(GS수퍼)</span>
								</th>
								<td>
						...
						<c:if test="${info.hpYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">고객 연락처(핸드폰)</span>
								</th>
								<td>
						...
						<c:if test="${info.emailYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">이메일</span>
								</th>
								<td>
						...
						<c:if test="${info.addressYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">주소</span>
								</th>
								<td>
									<div>
									...
									<p class="mt10">
										<input type="text" name="adr_dtl" title="상세주소" class="inputType4" maxlength="100" placeholder="상세주소" />
									</p>
								</td>
							</tr>
						</c:if>
						...
						<c:if test="${info.calltimeYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">상담 가능 시간대</span>
								</th>
								<td>
						...
						<c:if test="${info.skbJoinYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">SKB 가입상태</span>
								</th>
								<td>
						...
						<c:if test="${info.lotCardNumYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">롯데카드번호</span>
								</th>
								<td>
						...
						<c:if test="${info.cstmrUnqNumYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">고객 고유번호</span>
								</th>
								<td>
						...
						<c:if test="${info.hanssMbspNumYn eq 'Y'}">
							<tr>
								<th scope="row">
									<span class="essen" title="필수입력">한샘 통합 멤버십</span>
								</th>
								<td>
	...
	<!-- 2019.06.27 웹 접수 페이지 개인정보 활용 동의 영역 상단삽입-->
