...
public class EMFStringUtil {
     ...
     *</pre>
     */
    public static double zeroDoubleConvert(Object src) {
...
/**
	...
	/**
     * <p>게시글 리스트에서 카테고리를 |||로 받아온것을 처리한다..</p>
     *
     * <pre>
     *   StringUtil.getCategoryParentName("20100901") = "2010-09-01"
     * </pre>
     *
     * @param categoryNm  입력받는 문자열 카테고리 3>카테고리 2>카테고리 1
     * @return 역변환된 이름 반환
     */
	public static String getCategoryParentName(String categoryNm) 
	{
		String[] arrCategory = categoryNm.split("~~~");
		String rtn = "";	
		if(arrCategory.length == 0)
		{
			return rtn;
		}
		for (int i = arrCategory.length - 1; i >= 0; i--) 
		{
			if(rtn.equals(""))
			{
				rtn = arrCategory[i];
	 ...
	 * 현재 날짜와 넘어온날짜의 비교
	 */
	public static long diffOfDate(String regdate){
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
		Date regDate = new Date ( );
		try
		{
			regDate = formatter.parse(regdate);
		}	  
		catch(Exception e)
		{
	    ...
	    long diff = regDate.getTime() - currentTime.getTime();
	    long diffHours = diff / (24 * 60 * 60 * 1000);
	    return diffHours;
	}
	
	/**
	 * 컨텐츠 리스트 상태값 표기 하기 위함.
	 * @param code 값은 0,1,2,3
	 * @return  작성중,승인요청,배포,배포(만기)
	 */
	public static String contentsStatus(String code){
		String result="";
		if(code.equals("0")){
			result="<font color='blue'>작성중</font>";
		}else if(code.equals("1")){
			result="승인요청";
		}else if(code.equals("2")){
			result="<font color='green'>배포</font>";
	 ...
	 * 단어 필터링
	 */
	public static String wordfilter(String strOrigin, String strWord)
	{
		String strRtn = nvl(strOrigin, "");
		String strReplace = "*";
		if(strWord.indexOf(",")>0)
		{
			String[] arrWord = split(strWord, ",");
			for (int i = 0; i < arrWord.length; i++) 
			{
				strRtn = strRtn.replace(arrWord[i], "*");
		    }
		}
		else
		{
			strRtn = strRtn.replace(strWord, "*");
		}		
		return strRtn;
	}
