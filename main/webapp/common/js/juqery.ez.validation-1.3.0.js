...
(function($){
	...
	var validation = {
		defaults : {
			...
			msg : {
				...
				idChk : "아이디는 영문 소문자, 숫자 조합 4~20자리까지 사용 가능합니다.",
				...
				lengthLimitChk : "{0}는(은) {1}~{2}자리입니다.\n 다시 한 번 확인해 주세요.", //"길이가 조건에 맞지 않습니다.",
				lengthLimit10Chk : "고객 고유번호는 10자리입니다.\n 다시 한 번 확인해 주세요.",
	...
	$.fn.validation = function(options){
		...
		msg.lengthLimitChk =  settings.validateType.lengthLimitChk.name + "는(은) " + ( settings.validateType.lengthLimitChk.min > 0 ? settings.validateType.lengthLimitChk.min : "" )
		+ ( settings.validateType.lengthLimitChk.max > 0 ? (settings.validateType.lengthLimitChk.min < settings.validateType.lengthLimitChk.max ? " ~ " + settings.validateType.lengthLimitChk.max : ( settings.validateType.lengthLimitChk.min <=0 ? settings.validateType.lengthLimitChk.max : "") ) + "자리": "" )  + "입니다.\n 다시 한 번 확인 해 주세요 ";
		...
		*/
		...
		var ConvertPhone = function(){
			$('input').each(function(){
				...
				{
					$(this).blur(function(event){ //커서 이동 문제로 keyup 이벤트 교체 
		...
		var validationType = {
			...
			CheckBox : function(obj, msg){    // validationType.CheckBox($(this), message);
				var check_ok = false;
				tagName = obj.attr('name');
		...
		$(".marketingAgree .check_list input[type='radio']").change(function(){
			if($("#mkInfo1").is(":checked")){
				mktAgree(1);
			}else if($("#mkInfo2").is(":checked")){
				mktAgree(0);
			}
		});
		$(".marketingAgree .check_list li input[type='checkbox']").change(function(){
			var l = $(".marketingAgree .check_list li input[type='checkbox']:checked").length;
			if(l == 0){
				$("#btn_submit_mask").removeClass("hide");
			}else{
				$("#btn_submit_mask").addClass("hide");
			}
		});
		$(".marketingAgree .check_list li").click(function(){
			$(".marketingAgree .check_list li input[type='checkbox']").addClass("checkbox notRequired");
			if($(this).children("input").is(":checked")){
				console.log("체크");
				$(this).attr("checked",true);
				$(this).removeClass("checkbox notRequired");
			}else{
				console.log("해제");
				$(this).attr("checked",false);
				$(this).addClass("checkbox notRequired");
			}
		});
	};
})(jQuery);
...
function mktAgree(v) {
	...
	$("#mkInfo1, #mkInfo2").attr("checked","");

	chk.removeAttr("checked");
	if(v == 1){
		$("#mkInfo1").attr("checked");
		chk.removeAttr("disabled");
		chk.removeClass("checkbox notRequired");
		$("#mkInfo1").attr("checked","checked");
		$("#btn_submit_mask").removeClass("hide");
	}else if(v == 0){
		$("#mkInfo2").attr("checked");
		chk.attr("disabled","disabled");
		chk.addClass("checkbox notRequired");
		$("#mkInfo2").attr("checked","checked");
		$("#btn_submit_mask").addClass("hide");
	}
}