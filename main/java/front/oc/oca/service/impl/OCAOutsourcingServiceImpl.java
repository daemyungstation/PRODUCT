...
public class OCAOutsourcingServiceImpl extends EmfAbstractService implements OCAOutsourcingService {
	...
	public boolean insertOutsourcingInf(EmfMap emfMap, HttpServletRequest request) throws Exception
	{
		...
		if( "DLIVE".equals(emfMap.get("b2bStts")) ) {
			//DLIVE 는 추가 상품에 대한 정보를 입력하지 않는다.
			return true;
		}
	 ...
	 * @throws 비지니스 로직이나 DAO 처리 중 에러가 발생할 경우 Exception을 Throw 한다.
	 */
	public boolean insertOutsourcingInfEle(EmfMap emfMap, HttpServletRequest request) throws Exception
	{
		
		// 중복가입 검사
		EmfMap overrapMap = this.getOverrapSidEle(emfMap);
		System.out.println("==210902==");
		System.out.println(overrapMap);
		...
		oCAOutsourcingDAO.insertOutsourcingInfEleInfo(emfMap);
		if(!"".equals(emfMap.getString("b2bCd")))
		{
			EmfMap b2bComCdMap = dMLifeService.selectB2BComCd(emfMap);
		...
		String prdctCd = (String) emfMap.remove("prdctCd");
		String prdctCd2 = (String) emfMap.remove("prdctCd2");
		String prdctCd3 = (String) emfMap.remove("prdctCd3");
		String idNo2 = (String) emfMap.remove("idNo2");
		String idNo3 = (String) emfMap.remove("idNo3");
		String discountCtn = (String) emfMap.remove("discountCtn");		
		String discountCtn2 = (String) emfMap.remove("discountCtn2");
		String discountCtn3 = (String) emfMap.remove("discountCtn3");
		String discountCtn_10 = (String) emfMap.remove("discountctn10");
		String discountCtn2_10 = (String) emfMap.remove("discountctn210");
		String discountCtn3_10 = (String) emfMap.remove("discountctn310");
		...
		if(prdctCd.equals("LGU10")){
			emfMap.put("discountCtn", discountCtn_10);
		}else{
			emfMap.put("discountCtn", discountCtn);
		}

		oCAOutsourcingDAO.insertOutsourcingInfEle(emfMap);
		...
		if(!"".equals(EMFStringUtil.nvl(prdctCd2, "")))
		{
			...
			emfMap.put("mainContType", emfMap.getString("mainContType2"));
			emfMap.put("idNo", idNo2);
			if(prdctCd2.equals("LGU10")){
				emfMap.put("discountCtn", discountCtn2_10);
			}else{
				emfMap.put("discountCtn", discountCtn2);
			}
		...
		if(!"".equals(EMFStringUtil.nvl(prdctCd3, "")))
		{
			...
			emfMap.put("idNo", idNo3);
			emfMap.put("discountCtn", discountCtn3);
			if(prdctCd3.equals("LGU10")){
				emfMap.put("discountCtn", discountCtn3_10);
			}else{
				emfMap.put("discountCtn", discountCtn3);
			}
			EmfMap cdDtlInf4 = cmmUseService.selectCmmCodeDetailInf(emfMap);
	 ...
	 * @throws 비지니스 로직이나 DAO 처리 중 에러가 발생할 경우 Exception을 Throw 한다.
     */
    public EmfMap selectOutsourcingLGUPageMngInfo(EmfMap emfMap) throws Exception
    {
    	return oCAOutsourcingDAO.selectOutsourcingPageMngInfo(emfMap);
    }	
	 ...
	 * @throws 비지니스 로직이나 DAO 처리 중 에러가 발생할 경우 Exception을 Throw 한다.
     */
    public EmfMap getOverrapSidEle(EmfMap emfMap) throws Exception
    {
    	return oCAOutsourcingDAO.selectOverrapSidEle(emfMap);
    }
